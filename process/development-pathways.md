...
**What it protects.** Fully realizes ORCA's open-source mission: open collaboration, transparency, external contribution, and early adoption from day one. Merging changes is much easier, since work lands upstream as soon as it proves useful to the open source project.  Research advances are used in practice and submitted papers can contain operational experience since code will be used in practice.  Evaluation results can therefore be more realistic and case studies can be included.  The problems the work solves are also easier to motivate with real world examples.  
